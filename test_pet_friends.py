...
def test_get_api_key_for_valid_user(email=valid_email, password=valid_password):
    """Проверяем что api ключа вернет статус 500"""
    # Отправляем запрос
...
def test_get_api_key_for_valid_user(email=valid_email, password=valid_password):
    """Проверяем что в результате будет содержать слово age"""
...
def test_add_new_pet_with_valid_data(name='4363454765487', animal_type='(*))*:%/*8+-!@#$%%^',
